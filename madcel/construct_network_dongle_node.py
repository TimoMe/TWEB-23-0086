...
def construct_network_dongle_node(infer_result_valid, infer_result_test, valid_nodes, test_nodes, network):
    ...
    return network, label_matrix


if __name__ == '__main__':
    # test functions
    import pickle as pkl
    g_mat = pkl.load(open('../../dataset/cmu/graph_mat_5.cmu.pkl', 'rb'))
    ...
    # valid & test nodes
    train_slice, valid_slice = 5685, 5685+1895
    valid_nodes = [i for i in range(train_slice, valid_slice)]
    test_nodes = [i for i in range(valid_slice, len(g.nodes))]
    g_new, label_mat = construct_network_dongle_node(infer_result_valid, infer_result_test, valid_nodes, test_nodes, g)

    # # save g_new as sparse matrix
    # with open('../../dataset/cmu/graph_mat_5_lr.pkl', 'wb') as f:
